...
if RUN_AS_TEST:
    import keloid_skin_FEM_highly_nonlinear

    u_msr_dom_func_original = keloid_skin_FEM_highly_nonlinear.out['u_msr_dom_func']

    u_msr_dom_vals = None
    x_msr_dom_vals = None

    ux_msr_pad_vals = keloid_skin_FEM_highly_nonlinear.out['ux_msr_pad_left_vals']
    fx_msr_pad_vals = keloid_skin_FEM_highly_nonlinear.out['fx_msr_pad_left_vals']

    V_generator = keloid_skin_FEM_highly_nonlinear.out['Vector Function Space']
    dx_material_generator = keloid_skin_FEM_highly_nonlinear.out['FEM domain']
...
import keloid_skin_mesh_44000
from keloid_skin_mesh_44000 import (
    mesh_domain,
    markers_domain,
    markers_boundary,
    id_markers_domain,
...
V = VectorFunctionSpace(mesh_domain, 'CG', 1)
V_msr_u = VectorFunctionSpace(mesh_domain, 'CG', 1)
...
result_plots_data = open('result_plots_data.txt', 'w')

time_steps = [1, 2, 5, 8, 10]
#time_steps = [1,]
...
for std_u_i, val_u in enumerate(std_u):
    for std_f_i, val_f in enumerate(std_f):
        ...
        for time_step_ in time_steps:

            U_abs_err_all_times = sum(U_abs_err[0:n_msr:time_step_])/sum(U_ref_assembled[0:n_msr:time_step_])
            F_rel_err = relative_error_force(fx_msr_pad_left_noisy[0:n_msr:time_step_], fx_msr_pad_vals[0:n_msr:time_step_])

            t_obs = tuple(range(0, n_msr, time_step_)) # TEST

            m_initial = [0.009,  0.018,
                         0.007,  0.18,
                         1e-04]
            ...
            if not has_converged:
                ...
                result_parameters_file.write('Did not converged for time step = ' + str(time_step_) + '\n')
            else:
                print('has converged')
                result_parameters_file.write(str(time_step_) + ', ' + str(U_abs_err_all_times) + ', ' + str(F_rel_err) + ', ' + str(m_forall[0:4])[1:-1] + '\n')
            ...
            if PLOT_RESULTS :
                ...
                plt.savefig('results/plots/' + str(val_u) + '_' + str(val_f) +'/obs_cost_for_each_obs_time_' + str(time_step_) + '.png')
                plt.savefig('results/plots/' + str(val_u) + '_' + str(val_f) +'/obs_cost_for_each_obs_time_' + str(time_step_) + '.eps')
            ...
            if PLOT_RESULTS:
                ...
                result_plots_data.write(str(time_step_) + '\n')
                result_plots_data.write('*** \n')
                for (item_1, item_2, item_3) in zip(FENICS_ux_msr_pad_left_abs,\
                                                    FENICS_fx_msr_pad_left_abs,
                                                    FENICS_fx_msr_pad_left_noisy_abs):
                    result_plots_data.write(str(item_1) + '\t' + str(item_2) + '\t'\
                                            + str(item_2) + '\n')
                result_plots_data.write('================================= \n')

                ax.plot(FENICS_ux_msr_pad_left_abs[0:n_msr:time_step_], FENICS_fx_msr_pad_left_noisy_abs[0:n_msr:time_step_], 'k.', linewidth=2)
                ax.plot(FENICS_ux_msr_pad_left_abs, FENICS_fx_msr_pad_left_abs, 'b', linewidth=2)
                ...
                ax.set_xlabel('Pad displacement (mm)', fontsize=14)
                ax.set_ylabel('Pad reaction force (N)', fontsize=14)

                plt.tight_layout()
                plt.savefig('results/plots/' + str(val_u) + '_' + str(val_f) +'/inverse_solution_' + str(time_step_) + '.png')
                plt.savefig('results/plots/' + str(val_u) + '_' + str(val_f) +'/inverse_solution_' + str(time_step_) + '.eps')

result_parameters_file.close()
result_plots_data.close()
