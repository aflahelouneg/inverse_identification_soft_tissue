...
COST_FOR_EACH_ITERATION_STUDY = True

### Fabricate measurements

ELEMENT_DEGREE = 2

if COST_FOR_EACH_ITERATION_STUDY:
    # As the study is focused on observing the cost and the parameters
    # covnergence, the low cost mesh is used.
    ELEMENT_DEGREE = 1
...
bcs = [DirichletBC(V, uD_msr_pad_left, markers_boundary,
                    id_markers_boundary['pad_one']),
       DirichletBC(V, uD_msr_pad_left, markers_boundary,
                    id_markers_boundary['pad_one_sensor']),
       DirichletBC(V, uD_msr_pad_right, markers_boundary,
...
out = {
    ...
    'fx_msr_pad_left_vals': fx_msr_pad_left, # To compare with inverse solution
    }
